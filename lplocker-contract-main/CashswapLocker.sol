...
contract UniswapV2Locker is Ownable, ReentrancyGuard {
  ...
  struct TokenLock {
    ...
    uint256 lockID; // lockID nonce per cswap pair
    address owner;
  }
  ...
  mapping(address => TokenLock[]) public tokenLocks; //map UniswapV2 pair to all its locks
  
  struct FeeStruct {
    uint256 ethFee; // Small eth fee to prevent spam on the platform
    IERCBurn secondaryFeeToken; // CSWAP or BLOCK
    uint256 secondaryTokenFee; // optional, CSWAP or BLOCK
    uint256 secondaryTokenDiscount; // discount on liquidity fee for burning secondaryToken
    uint256 liquidityFee; // fee on UniswapV2 liquidity tokens
  ...
  /**
   * @notice set the migrator contract which allows locked lp tokens to be migrated to UniswapV2 next version
   */
   ...
   * @param _lpToken the UniswapV2 token address
   * @param _amount amount of LP tokens to lock
   * @param _unlock_date the unix timestamp (in seconds) until unlock
   * @param _referral the referrer address if any or address(0) for none
   * @param _fee_in_eth fees can be paid in eth or in a secondary token such as CSWAP with a discount on UniswapV2 tokens
  ...
  function lockLPToken (address _lpToken, uint256 _amount, uint256 _unlock_date, address payable _referral, bool _fee_in_eth, address payable _withdrawer) external payable nonReentrant {
    ...
    // ensure this pair is a UniswapV2 pair by querying the factory
    IUniswapV2Pair lpair = IUniswapV2Pair(address(_lpToken));
    address factoryPairAddress = UniswapV2Factory.getPair(lpair.token0(), lpair.token1());
    require(factoryPairAddress == address(_lpToken), 'NOT CSWAP');
    ...
    // record the lock for the UniswapV2pair
  ...
  function relock (address _lpToken, uint256 _index, uint256 _lockID, uint256 _unlock_date) external nonReentrant {
    ...
    // send UniswapV2 fee to dev address
    TransferHelper.safeTransfer(_lpToken, devaddr, liquidityFee);
  }
  ...
  /**
   * @notice migrates liquidity to UniswapV2 next version
   */
