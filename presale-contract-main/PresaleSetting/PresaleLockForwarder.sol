...
contract PresaleLockForwarder is Ownable {
    ...
    IUniswapV2Locker public UniswapV2_LOCKER;
    IUniswapV2Factory public UniswapV2_FACTORY;
    
    constructor() public {
        PRESALE_FACTORY = IPresaleFactory(0x647337276560f1Bd66A47780d9699216452f74CC);
        UniswapV2_LOCKER = IUniswapV2Locker(0x1992C19149350c0a33548e32b0f88B7CB2fd29EA);
        UniswapV2_FACTORY = IUniswapV2Factory(0xCC2FF6957da8E33d19472150301b7D65ACc83f5C);
    }
    ...
    */
    function UniswapV2PairIsInitialised (address _token0, address _token1) public view returns (bool) {
        address pairAddress = UniswapV2_FACTORY.getPair(_token0, _token1);
    ...
    function lockLiquidity (IERC20 _baseToken, IERC20 _saleToken, uint256 _baseAmount, uint256 _saleAmount, uint256 _unlock_date, address payable _withdrawer) external {
        ...
        address pair = UniswapV2_FACTORY.getPair(address(_baseToken), address(_saleToken));
        if (pair == address(0)) {
            UniswapV2_FACTORY.createPair(address(_baseToken), address(_saleToken));
            pair = UniswapV2_FACTORY.getPair(address(_baseToken), address(_saleToken));
        }
        ...
        TransferHelper.safeApprove(pair, address(UniswapV2_LOCKER), totalLPTokensMinted);
        uint256 unlock_date = _unlock_date > 9999999999 ? 9999999999 : _unlock_date;
        UniswapV2_LOCKER.lockLPToken(pair, totalLPTokensMinted, unlock_date, address(0), true, _withdrawer);
    }
