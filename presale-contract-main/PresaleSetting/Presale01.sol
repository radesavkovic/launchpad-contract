...
// ALL RIGHTS RESERVED
// BANCC reserves all rights on this code. You may NOT copy these contracts.
...
/**
  Allows a decentralised presale to take place, and on success creates a UniswapV2 pair and locks liquidity on BANCC.
...
interface IPresaleLockForwarder {
    ...
    function UniswapV2PairIsInitialised (address _token0, address _token1) external view returns (bool);
}
...
contract Presale01 is ReentrancyGuard {
  ...
  struct PresaleInfo {
    ...
    uint256 LISTING_RATE; // fixed rate at which the token will list on UniswapV2
    uint256 START_BLOCK;
    uint256 END_BLOCK;
  ...
  struct PresaleFeeInfo {
    uint256 BANCC_BASE_FEE; // divided by 1000
    uint256 BANCC_TOKEN_FEE; // divided by 1000
  ...
  address BANCC_DEV_ADDRESS;
  IUniswapV2Factory public CASH_FACTORY;
  IWETH public WETH;
  ...
  constructor(address _presaleGenerator) public {
    ...
    CASH_FACTORY = IUniswapV2Factory(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73);
    ...
    BANCC_DEV_ADDRESS = 0xAA3d85aD9D128DFECb55424085754F6dFa643eb1;
  }
  ...
  function init2 (
    ...
    uint256 _BANCCBaseFee,
    uint256 _BANCCTokenFee,
    ...
    address payable _referralAddress
      ...
      require(msg.sender == PRESALE_GENERATOR, 'FORBIDDEN');
      // require(!PRESALE_LOCK_FORWARDER.UniswapV2PairIsInitialised(address(_presaleToken), address(_baseToken)), 'PAIR INITIALISED');
      ...
      PRESALE_FEE_INFO.BANCC_BASE_FEE = _BANCCBaseFee;
      PRESALE_FEE_INFO.BANCC_TOKEN_FEE = _BANCCTokenFee;
  ...
  function userDeposit (uint256 _amount) external payable nonReentrant {
    ...
    bool userHoldsBANCCTokens = PRESALE_SETTINGS.userHoldsSufficientRound1Token(msg.sender);
    if (block.number < PRESALE_INFO.START_BLOCK + STATUS.ROUND1_LENGTH) { // 276 blocks = 1 hour
        require(userHoldsBANCCTokens, 'INSUFFICENT ROUND 1 TOKEN BALANCE');
    } else if (STATUS.WHITELIST_ONLY && !userHoldsBANCCTokens) {
      // Round 2
  ...
  // If the pair already exists on UniswapV2 and it contains the presale token as liquidity 
  ...
  function forceFailIfPairExists () external {
    ...
    if (PRESALE_LOCK_FORWARDER.UniswapV2PairIsInitialised(address(PRESALE_INFO.S_TOKEN), address(PRESALE_INFO.B_TOKEN))) {
        STATUS.FORCE_FAILED = true;
    }
  }
  
  // if something goes wrong in LP generation
  function forceFailByBANCC () external {
      require(msg.sender == BANCC_DEV_ADDRESS);
      STATUS.FORCE_FAILED = true;
  }
  ...
  function addLiquidity() external nonReentrant {
    ...
    uint256 BANCCBaseFee = STATUS.TOTAL_BASE_COLLECTED.mul(PRESALE_FEE_INFO.BANCC_BASE_FEE).div(1000);
    
    // base token liquidity
    uint256 baseLiquidity = STATUS.TOTAL_BASE_COLLECTED.sub(BANCCBaseFee).mul(PRESALE_INFO.LIQUIDITY_PERCENT).div(1000);
    if (PRESALE_INFO.PRESALE_IN_ETH) {
    ...
    // transfer fees
    uint256 BANCCTokenFee = STATUS.TOTAL_TOKENS_SOLD.mul(PRESALE_FEE_INFO.BANCC_TOKEN_FEE).div(1000);
    // referrals are checked for validity in the presale generator
    if (PRESALE_FEE_INFO.REFERRAL_FEE_ADDRESS != address(0)) {
        // Base token fee
        uint256 referralBaseFee = BANCCBaseFee.mul(PRESALE_FEE_INFO.REFERRAL_FEE).div(1000);
        ...
        BANCCBaseFee = BANCCBaseFee.sub(referralBaseFee);
        // Token fee
        uint256 referralTokenFee = BANCCTokenFee.mul(PRESALE_FEE_INFO.REFERRAL_FEE).div(1000);
        TransferHelper.safeTransfer(address(PRESALE_INFO.S_TOKEN), PRESALE_FEE_INFO.REFERRAL_FEE_ADDRESS, referralTokenFee);
        BANCCTokenFee = BANCCTokenFee.sub(referralTokenFee);
    }
    TransferHelper.safeTransferBaseToken(address(PRESALE_INFO.B_TOKEN), PRESALE_FEE_INFO.BASE_FEE_ADDRESS, BANCCBaseFee, !PRESALE_INFO.PRESALE_IN_ETH);
    TransferHelper.safeTransfer(address(PRESALE_INFO.S_TOKEN), PRESALE_FEE_INFO.TOKEN_FEE_ADDRESS, BANCCTokenFee);
